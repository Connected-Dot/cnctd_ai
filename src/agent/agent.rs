...
impl<'a> Agent<'a> {
    ...
    pub async fn run(
        ...
        let executor = AgentExecutor::new(self.client, &self.config, self.mcp_client.clone());
        executor.execute(task, request).await
    }

    /// Run the agent with a simple task (no pre-configured request)
    /// If an MCP client is configured, automatically loads tools
    pub async fn run_simple(&self, task: impl Into<String>) -> Result<AgentTrace, Error> {
        ...
        // Auto-discover and load tools from MCP client if available
...
impl<'a> AgentBuilder<'a> {
    ...
    pub fn mcp_client(mut self, client: Arc<McpClient>) -> Self {
        self.mcp_client = Some(client);
        self
    }
